...
def outcome_frequency(session_df_correct, session_obj):
	...
	for c_index, condition in enumerate(sorted(session_df_correct['condition'].unique())):
		...
		for f_index, fractal in enumerate(sorted(session_obj.stim_labels)):
			session_df_fractal_condition = session_df_condition[session_df_condition['fractal_chosen'] == fractal]
